...
# -----------------------------------------------------------------------------
#    Modidied by: Yang Li & Daniel Asante
# -----------------------------------------------------------------------------
...
import torch
import transformers  
import methods.basis_selection as bs
...
class ModelArguments:
    ...
    decompressed_model_path: str = field(default=None, metadata={"help": "Path to the directory containing the decompressed model checkpt after part 2"})
...
def pt_fsdp_state_dict(model: torch.nn.Module) -> Dict[str, Any]:
...
def smartoutput_dir_second_tokenizer_and_embedding_resize(
...
def train():
    ...
    print("The mode is loaded in the type:", model.dtype)
    ...
    dtype = model.dtype
    with open(
        os.path.join(model_args.part_1_output_path, "dim.json"), "r"
    ) as file:
        dim_dict = json.load(file)
    model = bs.convert_from_base_to_factorized_model(
        model,
        dim_dict,
    ).cuda()
    checkpoint = torch.load(
        os.path.join(model_args.part_1_output_path, "pytorch_model.bin"),
        map_location="cpu",
    )
    ...
    if rank == 0:
        os.makedirs(model_args.decompressed_model_path, exist_ok=True)
        trainer._save(model_args.decompressed_model_path, state_dict=cpu_state)
        model = bs.unwrap_fsdp(model)
        torch.cuda.empty_cache()
        bs.decompress_state(model, cpu_state)
        
        try:
            os.remove(model_args.decompressed_model_path + "/pytorch_model.bin")
        except OSError:
            pass
        torch.save(cpu_state, model_args.decompressed_model_path + "/pytorch_model.bin")
        print("Decompressed model saved successfully")
